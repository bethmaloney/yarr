...
const ENRICHED_EVENTS = [
  { kind: "session_started", session_id: "sess-abc-123", _ts: 1741336200000 },
  { kind: "iteration_started", iteration: 1, _ts: 1741336201000 },
  { kind: "assistant_text", iteration: 1, text: "Analyzing the code...", _ts: 1741336205000 },
  ...
  { kind: "assistant_text", iteration: 2, text: "Implementing the fix...", _ts: 1741336235000 },
...
test.describe("RunDetail — iteration groups with token data", () => {
  test("iteration group shows token counts", async ({ page, mockTauri }) => {
    ...
    const iter1Header = page.locator(".iteration-header", { hasText: "Iteration 1" });
    await expect(iter1Header).toBeVisible();
    // The stats format is: "30,000 in / 3,200 out"
    await expect(iter1Header.locator(".iteration-stats")).toContainText("30,000 in");
    await expect(iter1Header.locator(".iteration-stats")).toContainText("3,200 out");

    // Iteration 2 header should display its token counts
    const iter2Header = page.locator(".iteration-header", { hasText: "Iteration 2" });
    await expect(iter2Header).toBeVisible();
    await expect(iter2Header.locator(".iteration-stats")).toContainText("62,000 in");
    await expect(iter2Header.locator(".iteration-stats")).toContainText("4,500 out");
  });

  test("context bar appears when context_window data exists", async ({ page, mockTauri }) => {
    await navigateToRunDetailWithEnrichedEvents(page, mockTauri);
  ...
  test("context bar shows correct percentage", async ({ page, mockTauri }) => {
    ...
    await expect(iter1Bar.locator(".context-bar-label")).toContainText("30k / 200k (15%)");
    ...
    await expect(iter2Bar.locator(".context-bar-label")).toContainText("62k / 200k (31%)");
  });
});
